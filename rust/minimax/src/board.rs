...
fn get_shift(dir: u8) -> fn(u64) -> u64 {
    match dir {
        0 => |x| x.wrapping_shl(8),           // N
        1 => |x| x.wrapping_shr(8),           // S
        2 => |x| x.wrapping_shl(1),           // W (Wait, standard representation: bit 0 is top-left usually, shl1 = right)
        3 => |x| x.wrapping_shr(1),           // E
        4 => |x| x.wrapping_shl(9),           // NW
        5 => |x| x.wrapping_shr(9),           // SE
        6 => |x| x.wrapping_shl(7),           // NE
        7 => |x| x.wrapping_shr(7),           // SW
        _ => unreachable!(),
    }
}
...
fn get_mask(dir: u8) -> u64 {
    match dir {
        ...
        2 | 4 | 6 => 0xFEFEFEFEFEFEFEFE, // Shift Left, so avoid rightmost col (col 0 conceptually)
        3 | 5 | 7 => 0x7F7F7F7F7F7F7F7F, // Shift Right, so avoid leftmost col
        _ => unreachable!(),
    }
}
